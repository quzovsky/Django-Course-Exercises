...
def book_list(request):
    ...
    author = request.GET.get('author') or 'a'
    name = request.GET.get('name') or 'k'

    # fill `.filter()` with query parameters
    all_books = Book.objects.filter(price__gte=min_price,price__lte=max_price,name__icontains=name,author__icontains=author)
